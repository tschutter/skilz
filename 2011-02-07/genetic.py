...
#
# genetic.py
# http://code.activestate.com/recipes/199121-a-simple-genetic-algorithm/
#
...
class Environment(object):
    def __init__(self, kind, population=None, size=100, maxgenerations=100,
        ...
        self.population.sort() #fix
        self.crossover_rate = crossover_rate
        self.mutation_rate = mutation_rate
        self.maxgenerations = maxgenerations
        self.generation = 0
        self.report()
        #print "end of __init__" #debug
    ...
    def _crossover(self):
        ...
        while len(next_population) < self.size:
            mate1 = self._select()
            #print "_crossover selected mate1 =", str(self.population.index(mate1))
            if random.random() < self.crossover_rate:
                mate2 = self._select()
                #print "_crossover selected mate2 =", str(self.population.index(mate1))
                offspring = mate1.crossover(mate2)
            else:
                #print "_crossover selected mate2 = mate1"
                offspring = [mate1.copy()]
            for individual in offspring:
                self._mutate(individual)
                individual.evaluate(self.optimum)
                next_population.append(individual)
        next_population.sort() #fix
        #for id, individual in enumerate(next_population): #debug
            #print "_crossover %4i:       " % id, individual     #debug
